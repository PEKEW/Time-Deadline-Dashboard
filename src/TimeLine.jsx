...
function TimePoint({ timePoint, position, today, ddl, canDrag, onDateChange, pixelsPerDay, prevDate, nextDate, offsetLevel, isConference = false }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`point-dot ${isConference ? 'conference-node' : ''}`}></div>
        ...
        <div className={infoClass}>
          <div className="point-name">{timePoint.name}</div>
          {isEditing ? (
            <input
              type="date"
              value={editValue}
            ...
            <div className="days-info">
              <div>{timePoint.date.toLocaleDateString('en-US')}</div>
              <div className="days-count red">
                {countdown}
              </div>
            </div>
          )}
...
function TimeLine({ program, today, onTimePointChange }) {
  ...
  {/* Main timeline line (red) - from today to last event */}
            ...
            return (
              <div key={`label-${timePoint.id}`}>
                ...
                <div
                  ...
                  style={{ left: `${pos2}px` }}
                >
                  <span className="red">{daysBetween} days apart</span>
                </div>
              </div>
            )
          }
